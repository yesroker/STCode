...
const FormatButton = () => {
  ...
  async function formatEditor() {
    const editorInstance =
      appContext?.currentMode === 'Code'
        ? appContext?.codeEditorRef.current
        : appContext?.diffEditorRef.current;
    if (editorInstance && 'getOriginalEditor' in editorInstance) {
      ...
      try {
        const formattedOriginalText = await prettier.format(originalText, PRETTIER_OPTIONS);
        originalModel?.setValue(formattedOriginalText);
      } catch (e) {
        console.error('格式化 Original (左侧) 编辑器失败:', e);
      }
    } else {
  ...
  return (
    <div className="border-b border-gray-700">
      <button
        ...
        className="w-8 h-8 hover:opacity-80 active:scale-95 transition-all duration-100 ease-in-out text-white hover:text-stone-400"
      >
        <svg viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="5211" className='mt-[2.7px]'>
          <path
